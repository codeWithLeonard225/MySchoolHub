...
const TimetableManager = () => {
    ...
    // 2. FETCH TIMETABLE ENTRIES (With ID De-duplication Fix)
    useEffect(() => {
        if (schoolId === "N/A") return;
        ...
        const unsub = onSnapshot(q, (snapshot) => {
            ...
            setTimetableList(Array.from(uniqueMap.values()));
        });

        return () => unsub();
    }, [schoolId]);

    // 3. AUTO-UPDATE SUBJECTS & HANDLE LUNCH
    useEffect(() => {
        const selectedClassObj = availableClasses.find((cls) => cls.className === formData.className);
    ...
    // 4. FILTERED LIST LOGIC (Memoized for performance)
    ...
    doc.text("OFFICIAL SCHOOL TIMETABLE", 14, 20);
    
    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text(`Class: ${filterClass || "All Classes"}`, 14, 30);
    doc.text(`Day: ${selectedDay}`, 14, 37);
    if(filterTeacher) doc.text(`Teacher: ${filterTeacher}`, 14, 44);

    // 2. Prepare Data
    const tableColumn = ["Period", "Time Slot", "Class", "Subject", "Teacher"];
    const tableRows = displayList.map(item => [
        item.period === "Lunch" ? "LUNCH" : `Period ${item.period}`,
        item.time,
        item.className,
        item.subject,
        item.teacher
    ...
    // 3. Generate Table using the imported autoTable function
    autoTable(doc, {
        startY: 50,
        head: [tableColumn],
        body: tableRows,
        ...
        // Custom styling for the "Lunch" row
        didParseCell: (data) => {
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm mb-8 border border-gray-200">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div className="flex flex-col gap-1">
                            ...
                            <select name="className" value={formData.className} onChange={handleChange} className="border p-2.5 rounded-lg text-sm bg-gray-50 focus:bg-white outline-teal-500">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex gap-2">
                                <select name="day" value={formData.day} onChange={handleChange} className="w-1/2 border p-2.5 rounded-lg text-sm bg-gray-50 outline-teal-500">
                                    {days.map(d => <option key={d} value={d}>{d}</option>)}
                                </select>
                                <select name="period" value={formData.period} onChange={handleChange} className="w-1/2 border p-2.5 rounded-lg text-sm bg-gray-50 outline-teal-500">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <select name="subject" value={formData.subject} onChange={handleChange} disabled={formData.period === "Lunch"} className="border p-2.5 rounded-lg text-sm bg-gray-50 disabled:opacity-50 outline-teal-500">
                                <option value="">-- Select --</option>
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <select name="teacher" value={formData.teacher} onChange={handleChange} disabled={formData.period === "Lunch"} className="border p-2.5 rounded-lg text-sm bg-gray-50 disabled:opacity-50 outline-teal-500">
                                <option value="">-- Select --</option>
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex gap-2">
                                <input type="time" name="startTime" value={formData.startTime} onChange={handleChange} className="w-1/2 border p-2 rounded-lg text-sm" />
                                <input type="time" name="endTime" value={formData.endTime} onChange={handleChange} className="w-1/2 border p-2 rounded-lg text-sm" />
                            </div>
                        </div>
                        <div className="lg:col-span-3 flex items-end gap-3">
                            <button type="submit" disabled={loading} className="flex-1 bg-teal-600 text-white font-bold py-3 rounded-lg hover:bg-teal-700 transition shadow-md uppercase text-xs">
                                {loading ? "Saving..." : editId ? "Update Schedule" : "Add to Timetable"}
                            </button>
                            {editId && (
                                <button type="button" onClick={() => {setEditId(null); setFormData({className: "", day: "Monday", period: "1", startTime: "08:00", endTime: "08:40", subject: "", teacher: ""});}} className="px-6 py-3 bg-gray-200 rounded-lg text-xs font-bold uppercase">
                                    Cancel
                                </button>
                            )}
                        </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="p-6 bg-white border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div className="text-center md:text-left">
                            <h2 className="text-xl font-black text-gray-800 uppercase">Master Schedule</h2>
                            <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">School Management Dashboard</p>
                        </div>
    ...
    <div className="flex gap-2 w-full md:w-auto">
        <select value={filterClass} onChange={(e) => setFilterClass(e.target.value)} className="flex-1 md:w-40 border p-2 rounded-lg bg-gray-50 text-xs font-bold">
            <option value="">🔍 All Classes</option>
            {availableClasses.map(c => <option key={c.id} value={c.className}>{c.className}</option>)}
        </select>
        <select value={filterTeacher} onChange={(e) => setFilterTeacher(e.target.value)} className="flex-1 md:w-40 border p-2 rounded-lg bg-gray-50 text-xs font-bold">
                                ...
                                {displayList.length === 0 ? (
                                    <tr>
                                        <td colSpan="5" className="py-20 text-center">
                                            <div className="text-gray-300 text-lg font-medium italic">No periods found for {selectedDay}</div>
                                            <button onClick={() => {setFilterClass(""); setFilterTeacher("");}} className="text-teal-500 text-xs font-bold mt-2 underline">Clear all filters</button>
                                        </td>
                                    </tr>
                                ) : (
                                    displayList.map((item, idx) => (
                                        <tr key={`${item.id}-${idx}`} className={`${item.period === "Lunch" ? "bg-orange-50/60" : "hover:bg-teal-50/30 transition-colors"}`}>
                                            <td className="px-6 py-4">
                                                <span className={`font-black text-xs ${item.period === "Lunch" ? "text-orange-600" : "text-teal-600"}`}>
                                                    {item.period === "Lunch" ? "🍱 BREAK" : `PERIOD ${item.period}`}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 font-bold text-gray-600">{item.time}</td>
                                            <td className="px-6 py-4">
                                                <span className="bg-gray-100 px-3 py-1 rounded text-[10px] font-black text-gray-500">{item.className}</span>
                                            </td>
                                            <td className="px-6 py-4">
                                                {item.period === "Lunch" ? (
                                                    <span className="text-orange-400 font-bold italic text-xs">Lunch Interval</span>
                                                ) : (
                                                    <div className="flex flex-col">
                                                        <span className="font-black text-gray-800 uppercase text-xs">{item.subject}</span>
                                                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">👨‍🏫 {item.teacher}</span>
                                                    </div>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">

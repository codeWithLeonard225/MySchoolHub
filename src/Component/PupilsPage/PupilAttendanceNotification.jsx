...
const IndividualReportCardTerm1 = () => {
  ...
  useEffect(() => {
    ...
    const pupilRegRef = query(collection(db, "PupilsReg"), where("studentID", "==", selectedPupil), where("schoolId", "==", schoolId));
  ...
  useEffect(() => {
    ...
    const pupilsRef = query(collection(db, "PupilsReg"), where("academicYear", "==", academicYear), where("class", "==", selectedClass), where("schoolId", "==", schoolId));

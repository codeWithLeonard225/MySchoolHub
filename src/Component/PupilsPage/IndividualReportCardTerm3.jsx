  ...
  const { subjects, reportRows, totalMarks, overallPercentage, overallRank } = useMemo(() => {
    ...
    for (const subject of [...new Set(classGradesData.map(d => d.subject))]) {
      ...
      for (const id of pupilIDs) {
        ...
        const t1 = studentSubjectGrades.find(g => g.test === "Test 5")?.grade || 0;
        const t2 = studentSubjectGrades.find(g => g.test === "Test 6")?.grade || 0;
    ...
    const subjectData = uniqueSubjects.map((subject) => {
      const t1 = pupilGradesData.find((g) => g.subject === subject && g.test === "Test 5")?.grade || 0;
      const t2 = pupilGradesData.find((g) => g.subject === subject && g.test === "Test 6")?.grade || 0;
    ...
    for (const id of pupilIDs) { 
        ...
        for (const subject of subjectsInClass) {
            const t1 = pupilGrades.find(g => g.subject === subject && g.test === "Test 5")?.grade || 0;
            const t2 = pupilGrades.find(g => g.subject === subject && g.test === "Test 6")?.grade || 0;

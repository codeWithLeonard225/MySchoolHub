...
const timetableCache = localforage.createInstance({
    ...
    storeName: "SSS1TimetableToday",
});

const attendanceCache = localforage.createInstance({
    name: "TimeTableTeacherCache",
    storeName: "SSS1AttendanceLog",
});
...
const SSS1periodAtt = () => {
    ...
    const availableClasses = useMemo(() => {
        return [...new Set(timetableToday
            .map((t) => t.className)
            .filter((className) => className && className.startsWith("SSS 1")) // Only SSS 1 in dropdown
        )];
    }, [timetableToday]);
    ...
    const filteredList = useMemo(() => {
        return timetableToday.filter((t) => {
            ...
            const isSSS1 = t.className && t.className.startsWith("SSS 1");

            // 2. Filter check: If a specific class is selected in dropdown, match it. 
            // Otherwise, just ensure it's any SSS 1 class.

...
const GradeSheetPage = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      const years = [
        ...new Set(snapshot.docs.map(doc => doc.data().academicYear).filter(Boolean))
      ];
      const classes = [
        ...new Set(snapshot.docs.map(doc => doc.data().className).filter(Boolean))
      ];
 ...
 // ✅ Fetch years and classes for the selected school
useEffect(() => {
  if (!schoolId || schoolId === "N/A") return;
  ...
  useEffect(() => {
    if (!selectedClass || !selectedTest || !academicYear || !schoolId) {
      setGradesData([]);
      return;
    }
